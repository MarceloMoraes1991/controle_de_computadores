<?php
    ...
    if (isset($_GET['search'])) {
        ...
        $sql = "SELECT * FROM controle_equipamentos WHERE nome_funcionario LIKE '%$search%' OR cpf LIKE '%$search%' OR equipamento LIKE '%$search%' OR modelo LIKE '%$search%' OR sn LIKE '%$search%' OR descricao LIKE '%$search%' LIMIT $offset, $registrosPorPagina";
    } else {
        // Consulta SQL original com LIMIT para selecionar apenas os dados da página atual
        $sql = "SELECT * FROM controle_equipamentos LIMIT $offset, $registrosPorPagina";
    }

    $result = mysqli_query($con, $sql);

    // Recupere o total de registros para calcular o número total de páginas
    $totalRegistrosQuery = mysqli_query($con, "SELECT COUNT(*) AS total FROM controle_equipamentos");
...
<body>
    ...
    <form method="GET" action="listar_equipamento.php" class="search-container">
    ...
    <ul class="pagination">
            <?php
                for ($pagina = 1; $pagina <= $totalPaginas; $pagina++) {
                    ...
                    echo "'><a href='listar_equipamento.php?pagina=$pagina'>$pagina</a></li>";
                }
            ?>
        </ul>
    ...
    <div class="fixed-action-btn">
        <a href="equipamento.php" class="btn-floating btn-large waves-effect waves-light"><i class="material-icons">add</i></a>
    </div>
</body>
</html>
